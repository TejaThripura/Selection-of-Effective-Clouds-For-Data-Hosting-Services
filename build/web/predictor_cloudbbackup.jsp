...
<body>
 ...
 <!-- MAIN CONTENT --> 
    ...
    <p><center><font size="5" color="white">Cloud B Back Up Files</font></center></p><br/>
    <style>
        th{
            color: green;
        }
...
try{ 
	Connection con = null;
        ...
        PreparedStatement pst=con.prepareStatement("select * from predictorcloudb");
	...
	while(rs.next()){
            %><tr>
                ...
                <td><textarea style=" width:40"><%=rs.getString(6)%></textarea></td>
                 <td><a href="predictor_cloudbbackup1.jsp?ownername=<%=rs.getString(3)%>&filename=<%=rs.getString(2)%>&part1=<%=rs.getString(5).replaceAll("\\+", "%2B")%>&part2=<%=rs.getString(6).replaceAll("\\+", "%2B")%>&skey=<%=rs.getString(7)%>&ownerdate=<%=rs.getString(4)%>&proxydate=<%=rs.getString(9)%>"><font color="#0d637d" size="3">Erasure</font></a> </td>
                  <td><a href="predictor_cloudbbackup11.jsp?ownername=<%=rs.getString(3)%>&filename=<%=rs.getString(2)%>&part1=<%=rs.getString(5).replaceAll("\\+", "%2B")%>&part2=<%=rs.getString(6).replaceAll("\\+", "%2B")%>&skey=<%=rs.getString(7)%>&ownerdate=<%=rs.getString(4)%>&proxydate=<%=rs.getString(9)%>"><font color="#0d637d" size="3">Replication</font></a> </td>
