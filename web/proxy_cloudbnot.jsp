...
<body>
       ...
       <script>alert('Data Backup Successfully...!');</script>
       <%}%>
  ...
  <ul>
    ...
    <li><a href="proxy_viewfiles.jsp">Upload Files</a></li>
    <li><a href="proxy_cloudanot.jsp">Cloud A Notifications</a></li>
    <li><a href="proxy_cloudbnot.jsp" class="selected">Cloud B Notifications</a></li>
 ...
 <!-- MAIN CONTENT --> 
    ...
    <p><center><font size="5" color="white">Cloud B Notifications </font></center></p><br/>
    <style>
        th{
            color: green;
        }
...
try{ 
	Connection con = null;
        ...
        PreparedStatement pst=con.prepareStatement("select * from cloudb_notification");
...
<%
	...
	while(rs.next()){
            %><tr>
                ...
                <td><textarea style=" width:40"><%=rs.getString(3)%></textarea></td>
                 <td><textarea style=" width:40"><%=rs.getString(4)%></textarea></td>
                  <td><a href="proxy_cloudbnot1.jsp?ownername=<%=rs.getString(2)%>&filename=<%=rs.getString(1)%>&data=<%=rs.getString(3).replaceAll("\\+", "%2B")%>&msg=<%=rs.getString(4)%>"><font color="#0d637d" size="3">Backup</font></a> </td>
